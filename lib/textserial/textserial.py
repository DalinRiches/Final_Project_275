...
class TextSerial(io.TextIOWrapper):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if sys.version_info.major >= 3 and sys.version_info.minor >= 3:
            ...
            super().__init__(io.BufferedRWPair(self.ser_in, self.ser_out, 1), encoding=encoding, errors=errors, newline=newline, line_buffering=line_buffering, write_through=write_through
                             )
        else:
            # no write_through in earlier pythons
            super().__init__(io.BufferedRWPair(self.ser_in, self.ser_out, 1), encoding=encoding, errors=errors, newline=newline, line_buffering=line_buffering
                             )
